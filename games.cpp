...
bool TTT::checkSpot(int spot)
{
	return spotCheck[spot / 3][spot % 3];
}

void TTT::replace(bool turn)
{
	int input;
	std::cin >> input;
	if (input >= 9 || input <= -1)
...
bool Connect4::checkSpot(int spot)
{
	return spotCheck[0][spot] == true;
}

void Connect4::replace(bool turn)
{
	int col;
	std::cin >> col;
...
void Game::updateStats(Player* currPlayerWin, Player* currPlayerLoss)
{
